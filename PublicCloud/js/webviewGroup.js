var webviewGroup = function(id, options) {
  ...
  this.nativeConfig = options.nativeConfig ? options.nativeConfig : {};
...
proto._initNativeView = function() {
  this.nativeView = new plus.nativeObj.View('__MUI_TAB_NATIVE', {
    'top': this.nativeConfig.top ? this.nativeConfig.top : '83px', //这个需要根据顶部导航及顶部选项卡高度自动调整
    'height': this.nativeConfig.height ? this.nativeConfig.height : (window.screen.height - 83) + "px",
    'left': '100%',
    'width': '100%',
    "backgroundColor": this.nativeConfig.backgroundColor ? this.nativeConfig.backgroundColor : "#ffffff",
  });
...
proto.getAllWebviews = function() {
  var i = 0;
  var wbvs = [];
  for(var key in this.webviewContexts) {
    var cnxt = this.webviewContexts[key];
    if(cnxt.webview) {
      wbvs[i] = cnxt.webview;
      i++;
    }
...
proto.getAllWebviewContexts = function() {
  var i = 0,
    cnxts = [];
  for(var key in this.webviewContexts) {
    var cnxt = this.webviewContexts[key];
