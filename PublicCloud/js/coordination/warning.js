(function($, doc) {
	...
	$.plusReady(function() {
		...
		var getAlertAndDealTicket = function() {
			//获取待诊断
			appFutureImpl.getTicketTasksByConditionAndPage(queryParams1, queryParams4Page, function(result, total, msg) {
				if(null != msg && !result) {
			...
			//获取已诊断工单条数
			appFutureImpl.getTicketTasksByConditionAndPage(queryParams2, queryParams4Page, function(result, total, msg) {
				if(null != msg && !result) {
					plus.nativeUI.toast(msg);
					return;
				}
		...
		var group = new webviewGroup(currentId, {
			...
			items: [{
				id: "main-subpage-coordination-list-processing.html",
				url: "main-subpage-coordination-list.html",
				extras: {
					queryStates: 5 //查询的是待诊断工单
				},
				...
				id: "main-subpage-coordination-list-processed.html",
				url: "main-subpage-coordination-list.html",
				extras: {
					queryStates: 6 //查询的是已诊断工单
				},
				initCallback: function() {
					//按需加载时，需要手动调用一下
					fireAlertAndTicket()
